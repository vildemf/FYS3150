...
T_list =        ['0.3', '0.5']#sys.argv[2].split(',')
...
legends = []
def plot_solution(T_list, d, Nt, Nx, method, cpp_program, titlename, forward, backward, crank, color, n):
    #legends = []
    x = linspace(0, d, Nx+1)
    dx = x[1]-x[0]
    for T in T_list:
        ...
        u_an = u_analytical(x, T)
        err = abs(u_an-u)
        plot(x, err*1E6, '%s' % (color))
        #legends.append('T=%.1f $\\Delta t$=%.0e' % (T, dt))
        legends.append('%s, T=%.1f $\\Delta t$=%.0e' % (n, T, dt))
    #override = {
    #'fontsize'            : 'large',
    #'verticalalignment'   : 'baseline',
    #'horizontalalignment' : 'center'
...
plot_solution(T_list, d, Nt, Nx, "forward", cpp_program, titlename, 1, 0, 0, 'r', 'F')    
plot_solution(T_list, d, Nt, Nx, "backward", cpp_program, titlename, 0, 1, 0, 'g', 'B') 
plot_solution(T_list, d, Nt, Nx, "crank", cpp_program, titlename, 0, 0, 1, 'b', 'C') 
...
xlabel("x", override)
ylabel("Absolute error in u(x, t=T) $\\times$%.0E" % (1E-6), override)
#title("%s \n Nt=%d Nx=%d $\\Delta x$=%.0e" % (Timeerror, Nt, Nx, dx))
title("Absolute Errors \n $N_x$=%.0E $N_t$=%.E" % (200, 50000))
legend(legends)#, loc="center right")#, bbox_to_anchor=(0.5, 1.05))
show()
